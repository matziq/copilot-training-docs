/* ============================================================
   CompTIA A+ Prep — Brooks  |  practice exam engine
   Renders timed exams, tracks answers, scores against the
   100-900 scale, and drives review mode.
   ============================================================ */
(function (global) {
    "use strict";
    ...
    function bank() { return global.AP_DATA || {}; }
    ...
    function Engine(opts) {
        this.o = opts;
        this.root = opts.el;
        this.core = opts.core === "core2" ? "core2" : "core1";
    ...
    Engine.prototype.load = function () {
        var saved = AP.store.get(this.key, null);
    ...
    Engine.prototype.save = function () { AP.store.set(this.key, this.state); };

    Engine.prototype.reset = function () {
        this.stopTimer();
        AP.store.remove(this.key);
    ...
    Engine.prototype.scoreSection = function (sec) {
        ...
        return {
            ...
            pct: Math.round(pct),
            scaled: this.isDrill() ? null : AP.rawToScaled(right, list.length),
    ...
    Engine.prototype.renderIntro = function () {
        ...
        // domain mix table, built from the questions themselves
        ...
        for (var i = 0; i < this.sections.length; i++) {
            ...
            for (var j = 0; j < list.length; j++) {
                var t = list[j].q.topic || "Untagged";
                mix[t] = (mix[t] || 0) + 1;
            }
        }

        var domains = AP.domainsFor(this.core);
        var rows = "";
        for (var d = 0; d < domains.length; d++) {
            var name = domains[d].name;
            var n = mix[name] || 0;
            if (!n) continue;
            var share = Math.round(n / totalQ * 100);
            rows += "<tr><td><strong>" + name + "</strong></td><td>" + n + " questions</td><td>" +
                share + "%</td><td>" + domains[d].weight + "%</td></tr>";
        }
        // anything tagged outside the standard domains still gets a row
        for (var k in mix) {
            if (!Object.prototype.hasOwnProperty.call(mix, k)) continue;
            var known = false;
            for (var dd = 0; dd < domains.length; dd++) if (domains[dd].name === k) known = true;
            if (!known) {
                rows += "<tr><td><strong>" + AP.escapeHtml(k) + "</strong></td><td>" + mix[k] +
                    " questions</td><td>" + Math.round(mix[k] / totalQ * 100) + "%</td><td>&mdash;</td></tr>";
            }
        }

        var pass = AP.passMark(this.core);
        var needed = AP.correctNeeded(this.core, totalQ);
        ...
        wrap.innerHTML =
            ...
            (rows
                ? '<div class="table-scroll"><table class="data-table"><thead><tr>' +
                  "<th>Domain</th><th>On this exam</th><th>Share here</th><th>Real exam weight</th>" +
                  "</tr></thead><tbody>" + rows +
            ...
            "hits zero, exactly like Pearson VUE. There is no penalty for a wrong answer on CompTIA exams, so " +
            "every question gets an answer even if it is a pure guess.</p>" +
            "<p style=\"margin-bottom:0\">Passing is <strong>" + pass + " on a 100\u2013900 scale</strong>, which lands " +
    ...
    Engine.prototype.renderSectionIntro = function () {
        ...
        var tip = this.isDrill()
            ...
            : "Read the last line of the question first: BEST, FIRST, and MOST LIKELY change the answer. If a question passes 90 seconds, flag it, answer it anyway, and move on.";
        ...
        wrap.innerHTML =
            ...
            '<div class="badge accent">' + (this.sections.length > 1
                ...
                : (this.core === "core2" ? "Core 2 \u00b7 220-1102" : "Core 1 \u00b7 220-1101")) + "</div>" +
    ...
    Engine.prototype.renderExam = function () {
        ...
        bar.innerHTML =
            ...
            '<span class="timer" id="clock">' + AP.fmtClock(this.state.timeLeft[sec.id]) + "</span>" +
    ...
    Engine.prototype.qCard = function (q, sec, review) {
        ...
        head.innerHTML = '<span class="q-num">' + q.n + "</span>" +
            (q.topic ? '<span class="badge">' + AP.escapeHtml(q.topic) + "</span>" : "") +
            (review && q.objective ? '<span class="badge accent">Obj ' + AP.escapeHtml(q.objective) + "</span>" : "");
        if (!review) {
    ...
    Engine.prototype.confirmSubmit = function () {
        ...
        var msg = blank > 0
            ? "You have " + blank + " unanswered question" + (blank === 1 ? "" : "s") +
              ". CompTIA does not penalize a wrong answer, so a guess is strictly better than a blank.\n\nSubmit anyway?"
            : "Submit and score?";
    ...
    Engine.prototype.renderResults = function () {
        ...
        var scaled = isDrill ? null : AP.rawToScaled(right, total);
        var pass = AP.passMark(this.core);
        ...
        if (isDrill) {
            ...
            var v = AP.verdict(scaled, this.core);
            var gap = scaled - pass;
            var shortBy = Math.max(1, AP.correctNeeded(this.core, total) - right);
            var deltaTxt = gap > 0
            ...
            wrap.appendChild(el("div", "score-hero",
                '<div class="composite-label">Estimated scaled score \u00b7 ' +
                (this.core === "core2" ? "Core 2 (220-1102)" : "Core 1 (220-1101)") + "</div>" +
                ...
                right + " of " + total + " correct (" + pct + "%). Scale runs 100\u2013900; passing is " + pass + ".</div>"));
        }
        ...
        for (var d = 0; d < domains.length; d++) {
            if (agg[domains[d].name]) {
                ordered.push({ name: domains[d].name, weight: domains[d].weight, s: agg[domains[d].name] });
        ...
        if (ordered.length > 1) {
            ...
            for (var o = 0; o < ordered.length; o++) {
                ...
                tiles.appendChild(el("div", "score-tile",
                    '<div class="sname">' + AP.escapeHtml(it.name) + "</div>" +
                    '<div class="sval">' + dp + "%</div>" +
                    '<div class="sraw">' + it.s.right + " / " + it.s.total + " correct" +
                    (it.weight ? " &middot; " + it.weight + "% of the real exam" : "") + "</div>" +
        ...
        for (var p = 0; p < results.length; p++) {
            ...
            paceRows += "<tr><td>" + AP.escapeHtml(results[p].sec.name) + "</td><td>" + sp + " of " +
        ...
        for (var w = 0; w < weak.length; w++) {
            ...
            weakHtml += '<div class="meter-row"><span class="mlabel">Objective ' + AP.escapeHtml(weak[w].name) + "</span>" +
        ...
        if (!weakHtml) {
            ...
            for (var y = 0; y < tw.length; y++) {
                ...
                weakHtml += '<div class="meter-row"><span class="mlabel">' + AP.escapeHtml(tw[y].name) + "</span>" +
        ...
        wrap.appendChild(el("div", "grid grid-2",
            ...
            '<p class="muted" style="margin-bottom:0;font-size:.88rem">The real exam gives you 90 minutes for up to ' +
            "90 questions. If you are burning the whole clock here, pacing is a separate problem from knowledge and " +
            "needs its own practice.</p></div>" +
        ...
        if (this.sections.length > 1) {
            for (var i = 0; i < this.sections.length; i++) {
                tabs += '<button class="btn sm ' + (i === idx ? "" : "secondary") + '" data-sec="' + i + '">' +
                    AP.escapeHtml(this.sections[i].name) + "</button>";
            }
        }
        head.innerHTML =
        ...
        mountDrill: function (opts) {
            ...
            if (!set) {
                opts.el.innerHTML = '<div class="card"><h3>Drill not found</h3><p>No drill set matches &quot;' +
                    AP.escapeHtml(opts.drillId) + '&quot;.</p></div>';
                return null;
            }
            var sectionObj = {
                id: "drill_" + set.id,
            ...
            return new Engine({
                ...
                title: set.title,
                blurb: set.blurb || "",
                core: set.core === "Core 2" ? "core2" : "core1",
